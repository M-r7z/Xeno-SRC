...
namespace XenoUI
{
	...
	{
		public string XenoVersion = "1.0.4";
		...
		private async void LoadSupportedVersion()
		{
			try
			{
				...
				string latestVersion = XenoVersion;
				if (latestVersion != XenoVersion)
				{
					MessageBox.Show("You can close this safely.", "Something went wrong, but it's ok.", MessageBoxButton.OK, MessageBoxImage.Warning);
				}
				SupportedVersion = await client.GetStringAsync("https://raw.githubusercontent.com/MaximumADHD/Roblox-Client-Tracker/refs/heads/roblox/version-guid.txt");
				SupportedVersion2 = await client.GetStringAsync("https://raw.githubusercontent.com/MaximumADHD/Roblox-Client-Tracker/refs/heads/roblox/version.txt");
			}
		...
		private async Task AddClientCheckBox(ClientInfo client)
		{
			...
			if (SupportedVersion != client.version && SupportedVersion2 != client.version)
			{
